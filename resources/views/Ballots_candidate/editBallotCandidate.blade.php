                ...
                <div class="row mb-5">
					<div class="col">
						<section class="card">
							...
							<div class="card-body">
								<form method="POST" action="/Constituency/update/{{$constituency->Constituency_id}}" class="form-horizontal form-bordered form-bordered">
                      				@csrf
                                    @method('PUT')
                                    <div class="form-group row">
                                        <label for="tags-input-multiple" class="col-lg-3 control-label text-lg-right pt-2">Constituency Name</label>
                                        <div class="col-lg-7">
                                            <input type="text" class="form-control" name="C_name" value="{{ $constituency->Constituency_name }}">
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        <label for="tags-input-multiple" class="col-lg-3 control-label text-lg-right pt-2">Constituency Code</label>
                                        <div class="col-lg-7">
                                            <input type="text" class="form-control" name="C_code" value="{{ $constituency->Constituency_code }}">
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        ...
                                        <div class="col-lg-7">
                                            <select type="text" class="form-control" name="C_type">
												@foreach($ctype as $c_type)
                                                    @if($constituency->Constituency_type == $c_type->Ctype_id)
                                                        <option value="{{$c_type->Ctype_id}}" selected>{{$c_type->Ctype_name}}</option>
                                                    @else
                                        ...
                                        <div class="col-lg-7">
                                            <select type="text" class="form-control" name="C_state">
												...
												@foreach($states as $state)
                                                    @if($constituency->State == $state->State_id)
									...
									<div class="form-group row">
										...
										<div class="col-lg-7">
											<select name="poscode[]" id="tags-input-multiple" multiple data-role="tagsinput" data-tag-class="badge badge-primary">
                                                @foreach(json_decode($constituency->C_Postcode, TRUE) as $postcode)
